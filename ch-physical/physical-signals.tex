...
This lesson introduces the fundamental concepts of multimedia from an
``outside the computer'' perspective. I define the term
...
natural and artificial means) and how we perceive them. I also
introduce a set of convenient mathematical tools for describing
signals. When you are done with this chapter, I hope you will
...
interpret as text. It is only very recently that computers have done
...
acquisition one. For brevity's sake, however, I will confine myself in
...
different kind of nerve cells arranged in well-structured layers. I
...
sections, I will outline first how computer systems can generate
...
When we eventually want to represent physical signals in the computer, perception of the signals will play an even more vital design role. For instance the sensors we use to capture light and sound are not nearly as advanced as the human ear or human eye. They contain various forms of noise that corrupt the \emph{integrity} of the signal. The difference between how we perceive \emph{physical signals} and how we perceive signals that have been \emph{captured by different sensors} will be of great importance to us. We will return to this concept later on.
...
\index{CRT|(}
\index{LCD}
CRTs here. A simplified CRT is sketched in figure~\ref{fg:crt}. It is
...
\emph{phosphors}: chemical which glow (emit visible light) when struck
\index{CRT!phosphor}
with electrons. Different phosphors emit different light wavelengths,
...
As we will see, it turns out that any periodic signal can be represented as a weighted sum of \emph{harmonically} 
\index{harmonics}
related sines and cosines (sinusoids at frequencies that are integer multiples of each other). If a periodic signal has a period of $T_0$, it can be represented by a weighted sum of sinusoids at harmonics of $T_0$, namely sinusoids with periods of $T_0$, $\frac{1}{2}T_0$, $\frac{1}{3}T_0$, $\frac{1}{4}T_0$, $\cdots$. This is potentially very powerful! It means that if we know the period of any repeating function, we can be represent it by a discrete set of weights (one weight for each harmonic). 
...
amplitude). Think of this vector rotating around the origin of the $x$-$y$ plane.
At any point in time, the vector makes an angle
$\theta(t)$ with the positive $x$ axis. The phase, $\phi$, is defined as $\phi = \theta(0)$
(i.e., the angle that the vector makes with the $x$-axis at $t=0$).  As figure~\ref{fg:phasor-components} shows,
 the $x$ and $y$ \emph{components} of the vector are cosine and sine functions.
...
considering components of the vectors), we didn't actually define what
the plane was. Let's imagine that our rotating vectors are in the
\emph{complex} plane, where $x$ is the real axis and $y$ is the
imaginary axis.  At any point in time, the location of a vector is
therefore a complex number:
\index{complex numbers|(emph}
...
a_1 e^{j\omega t} + a_2 e^{j(\omega+\delta)t} &=
 ...
 & = \underbrace{e^{j\omega t}}_{high frequency} \underbrace{(a_1 + a_2 e^{j\delta t})}_{low frequency}
\end{align}
...
amplitude that is a function of time: $a_1 + a_2 e^{j\delta
  t}$. Figure~\ref{fg:beating} presents the graphical version of this,
which looks like a top view of the amusement park ``octopus''
ride. There are times when the $e^{j\delta t}$ adds a factor of up to
$a_2$ to the first sinusoid and times when it subtracts a factor of up
to $a_2$. The result is the beating waveform seen in
figure~\ref{fg:beating-waveform} (in this case, the result of adding
10Hz and a 12Hz sinusoids, which can be seen to produce a 2Hz ``beat
frequency'').
...
\url{http://courses.washington.edu/css457/ebook/beating.m}.
\index{MATLAB code!beating}
\item Audio files to demonstrate beating: a 1000Hz
sine wave at
\url{http://courses.washington.edu/css457/ebook/1000Hz.au}, a 1005Hz
sine
wave at \url{http://courses.washington.edu/css457/ebook/1000Hz.au}, and
their
sum, at \url{http://courses.washington.edu/css457/ebook/beating.au}.
...
continuous time variable $t$ with period $T$,
\begin{equation}
...
projection of $f(t)$ onto the basis $e^{jk\omega_0 t}$, just like
...
\index{Fourier coefficients|emph}
\emph{frequency content} or \emph{spectrum} of $f(t)$, $k=0, \pm 1,
\pm 2, \ldots, \pm\infty$. This spectrum is just like the familiar
shorthand of writing a vector as a column or row of numbers, with the
understanding that each is the coefficient of one of the basis vectors
(i.e., when we write $\mathbf{v} = [1 2 3]$, what we actually mean is
$\mathbf{v} = 1 \vec{\mathbf{x}} + 2 \vec{\mathbf{y}} + 3
\vec{\mathbf{z}}$).  This is the signal's representation in the
\emph{frequency domain}, where frequency is the coordinate (in other
words, the signal expressed as a function of frequency).  Similar,
$f(t)$ is the signal's \emph{time domain} representation, where time
is the coordinate. Notice that the Fourier series transforms a finite
(periodic), continuous signal in the time domain into an infinite,
discrete spectrum in the frequency domain.
...
Since $c_k$ is the spectrum of $f(t)$, $|c_k|^2$ is the \emph{power
  spectrum}. Equation~(\ref{eq:real-f-pspec}) tell us that the power
\index{power spectrum} spectrum is a symmetric (or ``even'') function
of frequency. Also when $f(t)$ is real we can further decompose the Fourier Series. Equation ~(\ref{eq:fs}) becomes
\begin{align}
f(t) &= \sum_{k=-\infty}^{\infty} c_k e^{j k\omega_0 t} \notag\\
&= c_0+\sum_{k=1}^{\infty} c_k e^{j k\omega_0 t}
+\sum_{k=-1}^{-\infty} c_k e^{jk\omega_0 t} 
&&\text{(split sum into parts)}
\notag\\
&= c_0+\sum_{k=1}^{\infty} c_k e^{jk\omega_0t}+
\sum_{k=1}^{\infty} c_{-k} e^{-jk\omega_0 t} 
&&\text{(distribute -1 into sum)}
\notag\\
&= c_0+\sum_{k=1}^{\infty} [c_k e^{jk\omega_0t}+
c_{-k} e^{-jk\omega_0 t}] 
...
-1. Let's do a bit more general of a case. I will consider a train of
...
an exponential is $\deriv{e^{at}}{t}$ is $ae^{at}$, then the
...
%the line spectrum of a periodic sequence of rectangular pulses as
...
frequency components \emph{do} change with the choice of $t_0$ (I just
